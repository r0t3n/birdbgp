...
function BirdBGP (options, callback) {
  ...
  if (callback && typeof(callback) !== 'function') {
    err = new Error('Callback is not a function');
    if (!this.emit(this._config.autoconnect == true ? 'connect' : 'open', err)) {
      throw new err;
    }
    return;
  }
  ...
  if (!fs.existsSync(this._config.path)) {
    err = new Error('Bird socket does not exist at path: ' + this._config.path);
    if (callback) {
      callback(err);
    } else if (!this.emit(this._config.autoconnect == true ? 'connect' : 'open', err)) {
      throw new err;
    }
    return;
  }
  ...
  if (!stat || !stat.isSocket()) {
    err = new Error('Bird socket is not a socket at: ' + this._config.path);
    if (callback) {
      callback(err);
    } else if (!this.emit(this._config.autoconnect == true ? 'connect' : 'open', err)) {
      throw new err;
...
BirdBGP.prototype._handleSocket = function () {
  ...
  socket.on('data', function (data) {
    ...
    if (Number.isInteger(parseInt(data[0]))) {
      ...
      self.emit('data', null, self._code, data.substr(1));
